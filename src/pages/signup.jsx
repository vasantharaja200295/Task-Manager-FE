    ...
    <div className=" h-screen w-screen flex flex-col items-center justify-center overflow-hidden">
      <div className=" h-fit w-[350px] border-[1px] rounded-sm p-3 box-border">
        ...
        <form onSubmit={handleSubmit(formSubmit)}>
            ...
            <Button className="w-full h-[30px] rounded-[5px] mt-5" disabled={isSubmitting}>{isSubmitting ? <Icon name="LoaderCircle" size={20} className=" animate-spin"/>:"Submit"}</Button>
            <div className=" text-xs w-full text-center mt-2 *:text-xs">Don't have an account ? <Button variant="link" className=" p-0 underline" onClick={()=>{navigate('/signup')}}>Sign Up</Button></div>
        </form>
      </div>
    </div>
  );
